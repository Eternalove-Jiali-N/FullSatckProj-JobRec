...
public class Recommendation {
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		// step 2 get all keywords and sort by count;
		...
		for (Entry<String, Integer> keyword : keywordList) {
			...
			// get by key, for example, "engineer"
			...
			for (Item item : items) {
				if (!favoritedItemIds.contains(item.getItemId()) && !visitedItemIds.contains(item.getItemId())) {
					...
					// initially everyone will be included, but later will check and preclude duplicity.
